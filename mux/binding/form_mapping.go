...
func setWithProperType(val string, value reflect.Value, field reflect.StructField) error {
	...
	case reflect.Struct:
		...
		return json.Unmarshal(strs.S(val), value.Addr().Interface())
	case reflect.Map:
		return json.Unmarshal(strs.S(val), value.Addr().Interface())
	default:
...
func setSlice(values []string, value reflect.Value, field reflect.StructField) error {
	slice := reflect.MakeSlice(value.Type(), len(values), len(values))
	err := setArray(values, slice, field)
	if err != nil {
		return err
	}
	value.Set(slice)
