...
func TestBindingFormMultipartForMap(t *testing.T) {
	...
	assert.Equal(t, 3.14, obj.MapFoo["pai"].(float64))
}
...
func testFormBindingEmbeddedStruct(t *testing.T, method, path, _, body, _ string) {
	b := Form
...
func testFormBindingIgnoreField(t *testing.T, method, path, _, body, _ string) {
	b := Form
	assert.Equal(t, "form", b.Name())
...
func testQueryBinding(t *testing.T, method, path, _, body, _ string) {
	b := Query
...
func testQueryBindingFail(t *testing.T, method, path, _, body, _ string) {
	b := Query
	assert.Equal(t, "query", b.Name())
...
func testQueryBindingBoolFail(t *testing.T, method, path, _, body, _ string) {
	b := Query
	assert.Equal(t, "query", b.Name())
